<div class="row g-3 mb-4 equal-height-selects align-items-end">
    <!-- Sucursal Origen -->
    <div class="col-md-4">
        <x-adminlte.select name="branch_id" label="Sucursal (Origen)" :options="$branches->pluck('name', 'id')->toArray()" :value="old('branch_id', $order->branch_id ?? auth()->user()->branch_id)" required />
    </div>
    ...
    <!-- Estado -->
    <div class="col-md-4">
        <x-adminlte.select name="status" label="Estado del Pedido" :options="$statusOptions" :value="old('status', $order->status->value ?? 0)" required />
    </div>
